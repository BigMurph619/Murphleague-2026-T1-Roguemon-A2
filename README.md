# Murphleague-2026-T1-Roguemon-A2

### | [Roguemon](#Roguemon) | [Tourney Awards](#Awards) | [Tourney Details](#Tourney-Details) | [Tourney Points](#Point-Breakdown) |

# Roguemon

[MurphLeague 2026](https://github.com/BigMurph619/Murphleague-2026) Tourney #1 is Roguemon Ascension 2. Base rules and information found [here](https://github.com/Crozwords/Roguemon).

RogueMon makes FRLG into a full Randomized Pokemon Roguelike. The goal is to win by locking every pokemon type and finishing a run without fainting. On your journey, you'll receive prizes to help you on your way while curses will hinder your path.

# Tourney Details

T1 is hosted by [BigMurph619](https://www.twitch.tv/bigmurph619). Dates: January 1st 11:59 PM EST - January 18th 11:59 PM EST. Sign up deadline is 24 hours before Tourney Start time and details on where to sign up will be available soon.

> You must be in the [MurphVerse discord](https://discord.gg/fNX4geCU) and all seeds must be live for the entire run and have a VOD available. Murphleague tourneys are free to enter and open to all entrants unless previously banned.

Competitors will select 6 types to run up to 20 attempts each in. Your highest scoring run from each type will be added together to get your cumulative score. You may NOT run any of your selected types again for the remainder of the Murphleague 2026 season. Typeless is banned from your 6 choices.

## Awards

Upon entering this tournament you will claim which charity you are playing for. BigMurph619 will be providing donation to each of the top 3 finishers. Additionally, any donations from the "Pledge Page" in Murph's discord will be added to the prize pool.

Competitors will also earn "League Points" towards the MurphLeague according to their respective placement. League Points determine eligibility and seeding for the Murphleague playoffs which has a larger prize pool including charity donations, an engraved trophy, and more.

## Point Breakdown
All points are worth 1 point unless denoted by (). Points are earned by advancing through segments and obtaining full-clear prizes. Segments earned via reward do not count towards points. Point is earned once clearing the section or using PC to force progression.
|Points| 
| :-----------------------------------: | 
|Lab|
|Forest|
|Rival 2|
|Brock|
|Route 3| 
|Mt. Moon|
|Mt. Moon Full Clear|
|Rival 3|
|Route 24/25|
|Misty|
|Route 6/11|
|Rival 4|
|Lt. Surge|
|Route 9/10 North|
|Rock Tunnel/Route 10 South|
|Rival 5|
|Route 8|
|Erika|
|Game Corner|
|Pokemon Tower|
|Cycling Road/18/19|
|Koga|
|Safari (Effectively 2 points for Koga)|
|Silph Co|
|Silph Co Full Clear|
|Sabrina|
|Route 21/Mansion|
|Blaine|
|Giovanni|
|Rival 7|
|Victory Road Full Clear|
|Lorelei|
|Bruno|
|Agatha(2)|
|Lance(2)|
|Champ(3)|

 > 40 Points available 

 > Tiebreakers will be decided as follows: most wins, furthest run, lowest BST of furthest run, lower BST total of all point scoring runs, coinflip.

|        Bonus Points       |
| :-----------------------: |
|Static Fanatic - Correctly guess the primary typing of one the static Pokemon (Snorlaxes/Ghost) and earn 1 bonus point towards your tourney total.|
|Curse conqueror - Whoever conquers the most curses from all of their scoring runs combined will earn 3 bonus points towards their tourney total.|
|Which Switch - Complete Surge's trash puzzle first try and earn 1 bonus point! (must have clip linked with score updates).|
|Evolution Premonition - Correctly guess the evo and provide a clip to earn 1 bonus point. May earn 1 point in each type (6 total) and the run must be live (no guessing from the log or replay).|

Good Luck Challengers.
